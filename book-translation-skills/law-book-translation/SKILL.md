...
| **P3 目录/结构** | （本合集定义，脚本见下） | **`extract_toc.py`**, `fix_heading_levels.py`（可选） | 从 PDF **目次页** 用视觉模型抽 **chapter / section / subsection**；序言、執筆者紹介等单独成章；bbox 仅作 **校正** 而非唯一依据 |
...
**说明：** 历史上单独的「4.5 结构重排」Skill 已弃用；**法条双语与标题层级**应在 **P2 修复 / P3 目录** 与 **P5 翻译输出** 中完成，不再单独设一阶段文件。
...
- `p4_terminology/` — `glossary.json` / `glossary_v2.json`（冻结后勿改键名随意增删）
- `p5_translate_chunks/` 或 **JSON 分块目录** — 翻译输入
- `p5_translated/` — 译文 `.md`、`translated.json`（若用结构化翻译）、`coverage_report.json`
...
2. 运行（从 `workspace/` 起，路径按实际 PDF 调整，注意：下面示例使用旧路径 p3_toc，实际应为 p3_toc）：
...
P6 由 `build_latex.py` 转为 `tcolorbox`；标签在模板内为「法条原文（日语）」「法条译文（中文）」。
...
| 要先定章节再译 | 先做 **P3 `extract_toc.py`**，再 P4→P5 |
| 发现译文漏段 | 查结构化 `translated.json` + `check_translation_coverage.py`，补翻失败 id |
| 只要排版 | P6→P7，输入为已定稿的 `p5_translated/ch*.md` |
